...
exports.emailLogin = function(req, res) {
  ...
  User.findOne({email: email}, function(err_find_user, user) {
    ...
    bcrypt.compare(pass, hash, function(err_comp_hash, result) {
      ...
      if(result) {
        if(rem) res.cookie('Token', service.createToken(user), { maxAge:999999999, secure:true, httpOnly:true, domain:'localhost', path:'/', sameSite: 'lax'});
        else res.cookie('Token', service.createToken(user), { secure:true, httpOnly:true, domain:'localhost', path:'/', sameSite: 'lax'});
